class ProtocolEntityPostCommunityMedia2 < ProtocolEntity2
  PREFIX = '12'
  ...
  def decode_entity(cmd, payload)
    ...
    arg3unpacked = [@args[2]].pack('H*')
    arg4unpacked = [@args[3]].pack('H*')

    @comm = arg3unpacked
    @post_body = arg4unpacked
    {
        community_name: @comm,
        post_body: @post_body
    }
  ...
  def self.from_entity(entity, created_by_address)
    ...
    return ProtocolEntityPostCommunityMedia2.new(ProtocolEntity::OP_PREFIX + PREFIX, payload, created_by_address)
  end
